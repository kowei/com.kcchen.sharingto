...
- (void)shareViaInternal:(CDVInvokedUrlCommand*)command
  ...
  for (NSString* filename in filenames) {
    UIImage* image = (UIImage *)[self getImage:filename];
    if (image != nil) {
...
- (void)openImage:(NSString *)imageName {
  UIImage* image = (UIImage *)[self getImage:imageName];
  if (image != nil) {
...
- (void)shareViaInstagram:(CDVInvokedUrlCommand*)command {
  ...
  UIImage* image = nil;
  for (NSString* filename in filenames) {
    image = (UIImage *)[self getImage:filename];
    break;
  }
...
- (void)shareViaWhatsApp:(CDVInvokedUrlCommand*)command {
  ...
  if (image != nil) {
    ...
    [UIImageJPEGRepresentation((UIImage *)image, 1.0) writeToFile:savePath atomically:YES];
    _documentInteractionController = [UIDocumentInteractionController interactionControllerWithURL:[NSURL fileURLWithPath:savePath]];
...
-(NSObject*)getImage: (NSString *)imageName {
  NSObject *image = nil;
  if (imageName != (id)[NSNull null]) {
    if ([imageName hasPrefix:@"http"]) {
      ...
      image = [UIImage imageNamed:imageName];
        ...
        if ([imageURL.pathExtension isEqual:@"gif"]) {
          image = imageData;
        } else {
